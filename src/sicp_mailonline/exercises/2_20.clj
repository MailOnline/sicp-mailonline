...
(defn same-parity-a [n & more]
  (letfn [(iter [in]
            (cond (empty? in) '()
                  (parity? n (first in)) (cons (first in) (iter (rest in)))
                  :else (iter (rest in))))]
    (iter (cons n more))))

;; a more idiomatic Clojure solution using filter
(defn same-parity-b [n & more]
  (filter (partial parity? n) (cons n more)))
